package org.axc.mexcooldowns.PluginUtils;

import java.util.Objects;
import org.axc.mexcooldowns.Mexcooldowns;
import org.bukkit.ChatColor;

public class Functions {
    public static String configPrefix() {
        return ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(Mexcooldowns.getInstance().getConfig().getString("prefix")));
    }
    public static String getConfigValue(String value) {
        return ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(Mexcooldowns.getInstance().getConfig().getString(value)));
    }
    public static void configReload() {
        Mexcooldowns.getInstance().reloadConfig();
    }
    public static String helpPlugin() {
        return ChatColor.translateAlternateColorCodes('&', "\n               &x&6&5&5&F&D&BM&x&6&7&5&F&D&6E&x&6&9&6&0&D&2X&x&6&B&6&0&C&DC&x&6&D&6&0&C&8o&x&6&F&6&1&C&3o&x&7&0&6&1&B&Fl&x&7&2&6&2&B&Ad&x&7&4&6&2&B&5o&x&7&6&6&2&B&0w&x&7&8&6&3&A&Cn&x&7&A&6&3&A&7s\n\n&7  /mexcooldowns reload &8[Plugin reload]\n ");
    }
}
